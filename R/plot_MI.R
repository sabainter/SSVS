#' Plot results of SSVS across Multiple Imputations
#'
#' This function creates a plot of SSVS coefficient estimates with the average, minimum, and maximum across imputations.
#'
...
#' Creates a plot of SSVS marginal inclusion probabilities (MIPs) with the average, minimum, and maximum across imputations with an optional threshold for highlighting meaningful predictors.
#'
...
#' @param title A character string specifying the plot title. Defaults to `"Marginal Inclusion Probabilities for SSVS-MI"`.
...
plot.mip <- function(data, threshold = 0.5, legend = TRUE, title = NULL, color = TRUE) {
  ...
  if (is.null(title)) {
    title <- "Marginal Inclusion Probabilities for SSVS-MI"
  }
